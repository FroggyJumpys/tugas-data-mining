"""
run_eda.py — Kondisi B: BERT + Cross-Entropy + EDA Augmentation
==============================================================
Fully self-contained — tidak ada external modules/
Menambahkan EDA Synonym Replacement pada kelas minoritas.

Output:
  - models/bert_b/best_model.pt
  - logs/bert_b_training_log.json
  - logs/sentiment_scores/sentiment_b.csv
  - results/condition_b/classification_report.txt
  - results/condition_b/confusion_matrix.png
  - results/condition_b/training_curve.png
"""
...
EDA_ALPHA = 0.1
EDA_TARGET_CLASSES = [1, 2, 3]   # Slightly Negative, Neutral, Slightly Positive
EDA_N_AUG = {1: 2, 2: 2, 3: 1}  # 2x for Slightly Neg & Neutral, 1x for Slightly Pos
EDA_SEED = 42
...
# ============================================================
# SECTION 4: EDA AUGMENTATION (OPTIMIZED)
# ============================================================
...
def plot_confusion_matrix(cm, save_path):
    ...
    plt.title('Confusion Matrix — Condition B (EDA)')
...
def main():
    print("=" * 60)
    print("KONDISI B — BERT + Cross-Entropy + EDA")
    print("=" * 60)
    print()
    ...
    model_dir = Path("models/bert_b")
    log_dir = Path("logs")
    sentiment_dir = log_dir / "sentiment_scores"
    result_dir = Path("results/condition_b")
    ...
    print("[2/6] Applying EDA augmentation on training set...")
    ...
    # 5. Train
    print("[5/6] Training BERT (5 epochs)...")
    training_result = train_bert(model, train_loader, val_loader, device, criterion=None, output_dir=model_dir, condition_name="B")
    print()
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("BERT SENTIMENT CLASSIFICATION — CONDITION B\n")
        f.write("=" * 60 + "\n\n")
    ...
    sentiment_path = sentiment_dir / "sentiment_b.csv"
    ...
    with open(log_dir / "bert_b_training_log.json", 'w', encoding='utf-8') as f:
        json.dump(log_data, f, indent=2)
    print(f"  Training log saved.")

    print()
    print("=" * 60)
    print("KONDISI B COMPLETE")
    print("=" * 60)
