"""
run_focal.py — Kondisi C: BERT + Focal Loss (Grid Search)
=========================================================
Fully self-contained — tidak ada external modules/
Grid search gamma ∈ {1.0, 2.0, 3.0}, pilih berdasarkan val macro F1 terbaik.

Output:
  - models/bert_c_best/best_model.pt
  - logs/bert_c_grid_search_log.json
  - logs/bert_c_best_training_log.json
  - logs/sentiment_scores/sentiment_c.csv
  - results/condition_c/classification_report.txt
  - results/condition_c/confusion_matrix.png
  - results/condition_c/training_curve.png
  - results/condition_c/grid_search_results.json
"""
...
# ============================================================
# SECTION 5: BERT DATASET & TRAINING
# ============================================================
...
def plot_confusion_matrix(cm, save_path):
    ...
    plt.title('Confusion Matrix — Condition C (Focal Loss)')
...
def main():
    print("=" * 60)
    print("KONDISI C — BERT + Focal Loss (Grid Search)")
    print("=" * 60)
    print()
    ...
    model_dir = Path("models/bert_c_best")
    log_dir = Path("logs")
    sentiment_dir = log_dir / "sentiment_scores"
    result_dir = Path("results/condition_c")
    ...
    # 3. Grid search over gamma values
    print("[3/7] Grid search over gamma values...")
    ...
    for gamma in tqdm(GAMMA_VALUES, desc="Gamma Search", unit="gamma", position=0):
        ...
        temp_model_dir = model_dir.parent / f"bert_c_gamma_{gamma}"
        training_result = train_bert(
            model, train_loader, val_loader, device,
            criterion=criterion,
            output_dir=temp_model_dir,
            condition_name=f"C_gamma_{gamma}"
        )
    ...
    print(f"[4/7] Best gamma: {best_gamma} (val macro F1: {best_result['best_val_f1']:.4f})")
    print()
    ...
    # 5. Load best model
    print(f"[5/7] Loading best model (gamma={best_gamma})...")
    best_temp_dir = model_dir.parent / f"bert_c_gamma_{best_gamma}"
    model = BertForSequenceClassification.from_pretrained(BERT_MODEL, num_labels=NUM_LABELS).to(device)
    print("  BERT model loaded (single GPU mode for stability).")
    ...
    with open(log_dir / "bert_c_best_training_log.json", 'w') as f:
        json.dump(best_log_data, f, indent=2)
    with open(log_dir / "bert_c_grid_search_log.json", 'w') as f:
        json.dump(grid_search_json, f, indent=2)
    print()
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("BERT SENTIMENT CLASSIFICATION — CONDITION C\n")
        f.write("=" * 60 + "\n\n")
    ...
    print("[7/7] Generating sentiment scores...")
    ...
    sentiment_path = sentiment_dir / "sentiment_c.csv"
